...
func (s *BetStore) Add(gameID int, userID int64, side string, mode string, items []ItemRef) int {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *BetStore) RemoveLastN(gameID int, userID int64, n int) {
	s.mu.Lock()
	defer s.mu.Unlock()
